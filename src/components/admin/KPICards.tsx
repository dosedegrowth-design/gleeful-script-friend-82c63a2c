    ...
    async function fetchStats() {
      try {
        ...
        const [
          ...
          supabase.from("mandatos").select("value_eur").eq("status", "ativo")
        ]);

        const activeMandatosCount = mandatos?.length || 0;
        const totalRevenue = mandatos?.reduce((acc, curr) => acc + Number(curr.value_eur || 0), 0) || 0;
